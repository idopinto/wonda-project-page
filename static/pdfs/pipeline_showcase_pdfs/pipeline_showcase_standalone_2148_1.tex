% Standalone WONDA pipeline figure — 2148_1.c
% Compile with: pdflatex pipeline_showcase_standalone_2148_1.tex
...
\end{tikzpicture}
\caption{\textsc{Wonda} pipeline on \texttt{2148\_1.c}. The raw verifier output ($V_0$) is normalized ($V_1$); the LLM then states the bitwise-accumulation closed form and drops redundant modulo constraints ($V_2$): \texttt{T == (1LL << i) - 1}, a $298\times$ character reduction with \textbf{1.5$\times$} verification speedup.}
\label{fig:pipeline_showcase_2148_1_c}
\end{figure*}
